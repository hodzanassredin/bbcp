...
В репо лежит готовый бинарь (собран на Ubuntu 24.04+, требует glibc >= 2.34).
Если `./run-bb64` падает с `version 'GLIBC_X.YY' not found` — просто
пересоберите командой выше под свою систему (нужен только gcc); локально
собранный бинарь будет работать на вашей glibc, сколь бы старой она ни была.
...
Курица-и-яйцо решается seed'ом: 32-битные `.ocf` компилятора лежат в репо
(`Dev/Code/*.ocf`), их грузит `dev0Linux`; дальше всё собирается из исходников.
