...
- `bbcp` — основное дерево. 32-битный мир (dev0) + исходники компилятора.
  Здесь правим код и собираем 32-битные .ocf для dev0. Расположение любое:
  все tools64-скрипты вычисляют корень по своему пути (`tools64/env64.sh`).
- `bbcp64use` — изолированное дерево для 64-битных артефактов.
...
- Редактируем ТОЛЬКО `*.odc.txt` (все в UTF-8, KB/OdcTextUtf8.md). `.odc` —
  ...
  мир должен быть собран). Мир мёртв (свежий клон / после wipe test64.sh):
  sync предупреждает и выходит 0, если все ожидающие .odc существуют
  (на клоне txt новее .odc только по mtime, содержимое то же) — go64
  компилирует имеющиеся .odc. Отсутствующий .odc при мёртвом мире — фатально.
